...
@section('content')
    ...
    <form action="/course/{{$course->id}}" method="post">
        <div class="form-group col-md-4">
            ...
            <input class="form-control" id="exampleInputEmail1" value="{{$course->title}}" name="title" type="text" aria-describedby="emailHelp" placeholder="">
        </div>
        <div class="form-group col-md-4">
            <label for="exampleInputEmail1">价格</label>
            <input class="form-control" id="exampleInputEmail1" value="{{$course->price}}" name="price" type="text" aria-describedby="emailHelp" placeholder="">
        </div>

        <div class="form-group col-md-4">
            <label for="exampleInputPassword1">图片</label><br>
            <img src="{{$course->pic}}" width="100" alt="">
            <div class="clearfix"></div>
            <input class="form-control" name="pic" value="{{$course->pic}}" id="exampleInputPassword1" type="text">
        </div>
        <div class="form-group col-md-12">
            <label for="exampleInputPassword1">内容</label>
            <div id="markdown">{{$course->content_m}}</div>
        </div>
        ...
        <div class="form-group col-md-12">
            ...
            <div id="labels">
                @if(count($tags))
                    @foreach($tags as $k=>$v)
                        <button type="button" class="btn btn-xs btn-info label @if($course->tags->has($v->id)) actived @endif" tid="{{$v->id}}"><span>{{$v->name}}</span></button>
                    @endforeach
                @else
